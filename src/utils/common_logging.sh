...
# --- TPU HELPERS ---
# Find TPU library on the VM
function find_tpu_library() {
  local TPU_NAME="$1"
  local TPU_ZONE="$2"
  local PROJECT_ID="$3"
  
  log "Looking for TPU library (libtpu.so) on VM..."
  local tpu_lib=$(gcloud compute tpus tpu-vm ssh "$TPU_NAME" \
    --zone="$TPU_ZONE" \
    --project="$PROJECT_ID" \
    --worker=all \
    --command="find / -name 'libtpu.so' 2>/dev/null | head -n 1" | tr -d '\r')
  
  if [[ -n "$tpu_lib" ]]; then
    log_success "Found TPU library at: $tpu_lib"
    echo "$tpu_lib"
    return 0
  else
    log_error "Could not find TPU library (libtpu.so) on VM"
    return 1
  fi
}
...
function verify_tpu_state() {
  ...
  log "Checking TPU VM state: $TPU_NAME"
