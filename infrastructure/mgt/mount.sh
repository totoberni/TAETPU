...
# Set default for HOST_MOUNT_DIR if not set
HOST_MOUNT_DIR="${HOST_MOUNT_DIR:-mount}"

# Upload to TPU VM using vmscp function
log "Uploading to TPU VM (project: ${PROJECT_ID}, zone: ${TPU_ZONE}, TPU: ${TPU_NAME})"

# Use vmscp for each file/directory in the temp directory rather than wildcards
for item in "$LOCAL_TEMP_DIR"/*; do
    if [ -e "$item" ]; then
        vmscp "$item" "~/${HOST_MOUNT_DIR#./}/"
    fi
done

# Copy from TPU VM file system to Docker container using vmssh function
