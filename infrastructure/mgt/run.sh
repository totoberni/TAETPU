...
WORKING_DIR="/app/mount/src"
FULL_SCRIPT_PATH="$WORKING_DIR/$SCRIPT_PATH"

log_info "Running script: $SCRIPT_PATH inside container"

# Run the script inside the container with proper working directory
gcloud compute tpus tpu-vm ssh ${TPU_NAME} \
    --zone=${TPU_ZONE} \
    --project=${PROJECT_ID} \
    --command="docker exec -w $WORKING_DIR taetpu-container python $FULL_SCRIPT_PATH $@"
